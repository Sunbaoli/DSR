...
opts.train = struct() ;
opts.train.derOutputs = {'edgeloss', 0,'SRloss',1} ;
opts.train.batchSize=120;
opts.train.numSubBatches = 4 ;
opts.train.gpus=[1];
opts.train.weightClipping=true;
% opts.train.weightClipping=false;
opts.train.theta=0.01;
opts.train.numEpochs = 350 ;
% opts.train.learningRate= 1*[logspace(-1,-2,20) logspace(-2,-3,20) logspace(-3,-4,20) logspace(-4,-5,20) logspace(-5,-6,20)...
...
opts.train.learningRate=1e-1*[1e-3*ones(1,10) ...
...
% while dataset is label=label-depth
    ...
    edges(:,:,2,:)=single(imdb.images.edges(:,:,1,batch)*0+50);
