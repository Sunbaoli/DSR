...
% -------------------------------------------------------------------------
opts.weightClipping=true;                                %weight clipping
opts.theta=0.005;
% ---------------------------------------------------------------------------
  ...
  switch net.params(p).trainMethod
    ...
    case 'gradient'
      ...
      if thisLR>0 || thisDecay>0
          %%%%%%%%%%%%%%%%weight clipping %%%%%%%%%%%%
          if params.weightClipping
              theta=params.theta/thisLR;
              parDer=weightClipping(parDer,theta);
          end
          %%%%%%%%%%%%%%%% adding by dxy %%%%%%%%%%%%%
...
%% avoid gradient explode, min(theta, abs(gradient) )
A(A>theta)  = theta;
A(A<-theta) = -theta;
